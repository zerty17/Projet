...
class Vecteur {
	public :
		...
		vector< double> get_vecteur() const;
		// autre
		...
		void set_coord(size_t i, double valeur); //permettant de modifier la valeur, on commence a partir de 0.
						 // d'une coordonnée du vecteur 

	
	private :
		vector <double > vect;
		size_t get_taille ()const ;
		double get_valeur(size_t i) const;// cette methode nous renvoie la valeur a la position i,
								 // sachant que l'on commence a numeroter le 1er element par 0.
};
	
vector< double> Vecteur::get_vecteur()const{ // on a besoin d'une methode qinous retourne le vecteur
	return vect;
}	

double Vecteur::get_valeur(size_t i)const{// cette méthode nous retourne la valeur a l'indice i, on commence a numeroter a partir de 0
	return vect[i];
}
	
size_t Vecteur::get_taille()const{// cette méthode nous retourne la taille des objets vecteur
	return vect.size();
}	
